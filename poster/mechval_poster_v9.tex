...
\block{M2: Baseline Separation}{
  ...
  \vspace{4mm}
  Automated interpretability scores on sparse autoencoders
  trained over randomly initialized transformers match
  those from trained models---the instrument cannot
  distinguish model structure from dictionary structure
  (Heap, 2025).

  \vspace{4mm}
  Unconstrained nonlinear alignment maps achieve 100\%
  interchange intervention accuracy on randomly initialized
  models. The metric measures the alignment map's flexibility,
  not the model's representational structure (Sutter, 2025).
}
}
...
\block{M3: Stability}{
  ...
  \vspace{4mm}
  The IOI circuit's headline ``87\% faithfulness'' spans
  \textbf{below 0\% to over 100\%} across six methodological
  choices (Miller, 2024). Symmetric token replacement and
  Gaussian noising detect \textbf{different sets of heads}
  for any fixed metric (Zhang, 2023). Across estimator and
  aggregation choices, the circuit ranges from 5 to 21
  edges while circuit error stays flat (Meloux, 2025).

  \vspace{4mm}
  Three independent groups, three ways to wiggle the
  analysis knobs, three different circuits from the same
...
\block{V1: Level Declaration}{
  ...
  \vspace{4mm}
  The Jacobian lens recovers a residual-stream subspace
  (implementational-statistical evidence). The paper says
  the subspace encodes what the model can report
  (representational claim). The narrative frames the
  subspace as a ``global workspace'' the model reasons in
  (computational claim)---three levels deep on evidence
  that reaches the representational level at best
  (Gurnee, 2026).
...
\block{V2: Level-Evidence Match}{
  ...
  \vspace{4mm}
  The ``refusal direction'' controls refusal across 13
  models in five families (Arditi, 2024). A later study
  finds a distinct harmfulness direction with different
  steering consequences---steering along harmfulness
  makes the model read harmless instructions as harmful,
  while steering along refusal elicits refusal without
  moving the judgment (Zhao, 2025). The lever and the
