% Mechanistic Validity — NEMI 2026 poster, v4
% Fixed title, separated takeaway from genetics block.
%
%   lualatex mechval_poster_grok_v4.tex
...
\title{%
  \parbox{0.92\linewidth}{\centering\bfseries
    ...
    A Validity Theory and Evidence Standard for Mechanistic Claims}}
...
\block{The Framework}{
  ...
  \begin{tabular}{lcp{0.52\linewidth}}
    ...
    \midrule
    Construct    & C1--C6  & Is the target concept well-defined? \\
...
% ════════════════════════════════════════════════════════
% ROW 4 — SOURCE DISCIPLINES (continued)
% ════════════════════════════════════════════════════════
\begin{columns}
\column{0.33}

\block{From Neuroscience \& Pharmacology}{
  Craver's mutual manipulability: a mechanism must be located at a
  level and testable by intervention at that level.
  \textbf{Weight-level claims need weight-level tests.}

  \vspace{4mm}
  Dose-response curves and the ORBITA trial: a circuit ablation is a
  maximal dose. \textbf{Graded interventions distinguish specific from
  nonspecific effects}---the dose-response curve between zero and
  complete ablation carries more information than the endpoint alone.
}
...
\block{From Genetics}{
  ...
  boundaries.} These fill gaps no existing mechanistic interpretability
  technique covers.
}
...
\block{Takeaway}{
  {\Large\bfseries A mechanistic claim is warranted only to the extent
  that every required link between concept, measurement, causal
  evidence, scope, and interpretation is supported.}
  ...
  \begin{itemize}[leftmargin=*, itemsep=3mm]
    ...
    \item \textbf{0/16} reach Validated; the best reaches Triangulated
    \item The field's most studied circuit stops two tiers short
    \item Criteria drawn from 6 source disciplines with a combined
      250+ years of methodological refinement
  \end{itemize}
