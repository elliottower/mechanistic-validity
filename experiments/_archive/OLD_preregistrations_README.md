...
## Migration note, added 2026-08-10

The five documents moved to one folder per experiment under `experiments/`, each beside the
code that runs it. Their text did not change: every file was re-hashed after the move and
all five still match the SHA-256 table above, so the freeze attestation covers the live
copies as well as the archived ones. The originals remain retrievable at commit `c643450`
of the predecessor repository.

| Frozen document | Now lives at |
|---|---|
| `double_dissociation_ioi_vs_greater_than.md` | `experiments/I6_double_dissociation/PREREG.md` |
| `name_frequency_and_sequence_length_confounds.md` | `experiments/I7_frequency_and_length_confounds/PREREG.md` |
| `circuit_replication_across_training_seeds.md` | `experiments/E4_replication_across_seeds/PREREG.md` |
| `head_level_minimality_retest.md` | `experiments/I3_head_level_minimality/PREREG.md` |
| `onset_offset_coupling.md` | `experiments/I11_onset_offset_coupling/PREREG.md` |

Four later documents — E1 intervention reach, I4 specificity, I10 rescue reversibility, and
M7 selection correction — were registered afterwards and are not covered by the table above.
They carry their own freeze statements in `experiments/2026-08-08_*/PREREG.md`.

The criterion labels cite `docs/paper/mechanistic_validity_v10.tex`, which is now
`paper/mechanistic_validity.tex`. Line numbers quoted in the frozen text refer to v10 and no
longer resolve; the criterion definitions themselves are unchanged.

A prior-art check on all nine questions is recorded in `experiments/SCOOP_CHECK.md`, together
with the errors it found in these frozen documents. Those errors are recorded rather than
edited away, since correcting a frozen document silently would defeat the freeze.
