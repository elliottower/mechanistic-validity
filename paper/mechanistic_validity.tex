...
COVID-19 ML & ML & I7 Confound ctrl & 0/61 models learned COVID pathology \\
\midrule
...
The framework evaluates such a claim through five layers:
...
\midrule
Interpretive inflation & Label implies intent or algorithm beyond what evidence shows & V4 Anthropo-\newline morphism check & ``World model'' for Othello, never contrastively tested against a causally-used state summary \\
\midrule
...
\midrule
Ignoring alternatives & Circuit works, but so does a different set of heads & I5 Rival mech.\newline exclusion & Meloux et al.\ find alternative IOI circuits with comparable faithfulness \\
\bottomrule
\end{tabular}
\end{table}
\endgroup
...
The internal-validity criteria are written in terms of interventions because interventions are the strongest evidence for necessity and sufficiency. Fields that cannot intervene establish I1 and I2 by other means: natural experiments, instrumental variables, comparison across cases. Those routes reach the same conclusion at the cost of assumptions the intervention did not need, so a claim should record which route it took. \Cref{tab:domain-routes} lists the substitutions and what each assumes.
...
Each metric takes a model, a task, and optionally an intervention as input, and produces a number as output. Metrics can be composed: one metric's output feeds into another. The framework organizes evidence into four evidence families; the current catalog defines 59 metrics across families A--F, drawing on interventionist, information-theoretic, structural, and generalization traditions of causation. The full inventory is in \Cref{appendix:metrics}.
...
Each claim is scored at its most lenient reading, and the lenient reading here is that these 26 heads carry the indirect-object computation. Rival subgraphs are compatible with it, since a second sufficient set establishes that the mechanism is not unique and uniqueness belongs to the stronger reading. Method disagreement (E1) caps the claim instead, and no reading escapes that. The localization is real and reproducible, and which subgraph constitutes \emph{the} mechanism is open. \Cref{appendix:ioi-audit} gives the full 36-criterion audit behind this verdict.

\paragraph{Probing Classifiers (Proposed).} Probing \citep{belinkov2022probing} trains a classifier on activations to test whether a property is decodable. The core limitation is one the survey itself names as its headline shortcoming: the probing framework ``may indicate correlations between intermediate representations \ldots\ and linguistic property $z$, but it does not tell us whether this property is involved in predictions of the model'' (\S4.3). Probe success establishes decodability, and high accuracy is equally consistent with genuine use, incidental separability in a high-dimensional space, and encoding of a confound. Three designed tests sharpen the ceiling. Control tasks show that popular probes are not selective---an MLP probe reaches 97.3\% on part-of-speech against 92.8\% on a task with randomized per-word-type labels, so most of the accuracy is the probe's own capacity \citep{hewitt2019designing}. Control datasets show that properties are recovered above chance even when they are non-discriminative for the original task. And amnesic probing shows that ``conventional probing performance is not correlated to task importance'' \citep{elazar2021amnesic}.
...
The audit in \Cref{sec:case-studies} scores published claims, but it invites an obvious objection: the criteria were designed by the same author who then applied them, so the verdicts could be retrofitted. \citet{meloux2026validating} state the general form of this problem sharply---``an explanation can be plausible (convincing to a human) without being faithful (an accurate account of the underlying mechanism), and users often prefer the former.'' A framework about validity has to survive being turned on itself, and plausibility to its own author is exactly the wrong evidence. We specify four checks. The first is the central one and is reported below; the remaining three are specified here and their status is stated explicitly.

\subsection{The historical validation audit}
\label{sec:historical-audit}

The framework makes a falsifiable prediction: \emph{a claim caps at the tier corresponding to its weakest validity dimension}. The prediction is testable against claims whose outcomes are already settled. We score historical claims on the evidence available at the time they were made, predict the ceiling each should have reached, and compare against what subsequently happened.

\paragraph{Matched controls.} Scoring failures alone would be post-hoc rationalization regardless of how well the framework fits. Every failed claim is therefore paired with a control: a claim from the same era, the same field, and the same evidence regime that \emph{held up}. The framework has discriminant power only if failures and controls differ in their weakest dimension. The strongest pairs share a dataset rather than merely a field---the height polygenic-adaptation signal against the height associations from the same consortium; ENCODE's 80\% biochemical-function figure against the conserved fraction reported in the same project; HDL against LDL as causal targets in the same epidemiological literature.

\paragraph{Blinding.} These cases are well known and their outcomes are known to us, so a promise to disregard that is not credible. We therefore score \emph{evidence structure} rather than cases. For each claim we extract a structured record from period-appropriate sources with an explicit cutoff at the date of the original claim, stating which evidence families were present, at what quality, with what sample sizes and controls, and omitting anything that identifies the case. A second rater scores the five dimensions from these records alone, blind to case identity and to outcomes. Rubric and records are frozen at a public commit hash before any join to outcomes. Redaction is feasible precisely because the framework scores structure: ``a cross-sectional association in $n\approx4{,}000$ with no interventional follow-up and no independent replication'' is scoreable without knowing which claim it is.

\paragraph{Hypotheses, frozen in advance.}
\begin{enumerate}[nosep]
\item \textbf{H1 (ceiling).} The predicted ceiling matches the tier the claim actually reached.
\item \textbf{H2 (discriminant).} Failures and matched controls differ in their weakest dimension. This is the substantive test; H1 without H2 is description.
\item \textbf{H3 (cross-field invariance).} Dimension-level verdicts behave the same way regardless of which field's evidence families were used to derive them (\Cref{sec:evidence-families}).
\item \textbf{H4 (which dimension).} Construct and internal validity carry the discriminant signal; measurement and external validity do not. HDL had excellent measurement and external validity and failed regardless.
\end{enumerate}

\paragraph{Status and results.}
\textcolor{red}{[RESULT: $n=20$ cases (10 failures, 10 matched controls) across mechanistic interpretability, genetic epidemiology, clinical prediction and neuroimaging. Exact-tier match rate with Clopper--Pearson CI; within-one-tier rate. H2: paired comparison of weakest-dimension score, failures vs.\ controls. H3: does the H2 effect hold within each field separately. H4: which dimension separates failures from controls. Report all four outcomes including nulls.]}

\textcolor{red}{[STATUS: protocol specified and pre-registered; audit not yet run. Ten matched pairs detects only a large effect---interpret a null as underpowered rather than as refutation.]}

\subsection{Remaining checks}

\paragraph{A ground-truth calibration.} The Validated tier is reachable only against a claim whose mechanism is known independently of the audit. Modular addition supplies one: a small transformer trained to generalization implements modular arithmetic through a Fourier construction stated in closed form \citep{nanda2023progress}, and \citet{zhong2023clock} show the same task admits a second circular-embedding algorithm---``Pizza''---alongside the original ``Clock,'' distinguished by gradient symmetricity and distance irrelevance rather than by anything in the Fourier domain. That distinction is what makes the setting a calibration target rather than merely a ground truth, and the reason is a measurement failure the framework should catch. Fourier structure does not discriminate the two: on the same Pizza model, the Clock functional form still explains 75.4--75.6\% of logit variance against Pizza's 99.2\%, so an instrument that certifies a mechanism at mid-70s fraction-of-variance-explained will certify the wrong one \citep[Table~2]{zhong2023clock}. Running the full measurement (M1--M7) and interpretive (V1--V5) suites on a trained model should therefore both certify the true mechanism at Validated and deny that tier to a claim asserting the other, on the strength of the discriminating instruments rather than the shared one. A framework that awards Validated to both, or to neither, is miscalibrated---and a framework that awards it on Fourier evidence alone has reproduced the M4 failure it exists to detect. \citet{meloux2026validating} provide a benchmark of ten simulated systems with hand-built ground-truth abstraction maps that makes the metric-level version of this calibration cheap to run; the claim-level version---does \emph{tier assignment} track ground truth, as distinct from whether a metric score does---remains ours to establish. \textcolor{red}{[STATUS: not yet run.]}

\paragraph{A pre-registered prospective audit.} Retrospective scoring cannot rule out retrofitting on its own. We therefore pre-register verdicts on newly published claims before their evidence is fully reviewed, freezing the predicted tier and its rationale at a public commit hash, and report how the prospective verdicts land. A verdict committed before the evidence is a severe test of the framework in exactly the sense the framework demands of the claims it audits---the same discipline, applied reflexively. \textcolor{red}{[STATUS: not yet run.]}

\paragraph{Inter-rater reliability.} Because aggregating criterion-level results to a tier involves judgment, two evaluators could assign different tiers to the same claim. The framework mitigates this by making the structured validity profile, not the tier, the primary output: disagreement localizes to individual, independently falsifiable criterion judgments rather than a holistic impression. Agreement is reported at the criterion level, where it matters, rather than only at the tier, using intraclass correlation with 95\% confidence intervals and the \citet{koo2016guideline} interpretive bands, following the protocol \citet{vandenben2026building} use for pipeline-induced variance in autointerpretability scores.

Two external results calibrate what to expect. \citet{joshi2026causality} annotate 186 claims across 50 papers for the gap between the evidence a method establishes and the claim its language asserts, and report inter-annotator agreement on their confidence rating at $\alpha = 0.11$---near chance. \citet{vandenben2026building} report intraclass correlations as low as $-0.049$ (95\% CI $[-0.094, -0.006]$) for autointerpretability scores under variation in which activating examples are sampled. Classifying scientific claims by evidential strength is a task on which trained annotators agree poorly, and any agreement statistic we report should be read against that floor rather than against an idealized one. \textcolor{red}{[RESULT: per-criterion ICC(2,1) with 95\% CIs, two independent raters, on a random subsample of the audited claims. Expect agreement on measurement criteria to exceed agreement on construct criteria; that gradient is itself a finding.]} \textcolor{red}{[STATUS: not yet run.]}
...
\midrule
A14 Onset--Offset Coupling & Whether a mechanism's measured strength tracks the capability in both directions: rising as the capability is acquired during training, falling as it is removed by unlearning or fine-tuning, and returning if the capability returns. Adapted from guideline 3 of the molecular Koch's postulates \citep{fredricks1996sequence} \\
\bottomrule
\end{tabular}
\medskip
...
\subsubsection{Full 35-Criteria Audit}

We audit the IOI circuit claim \citep{wang2023interpretability} against all 36 criteria to demonstrate what the framework produces in practice. The claim, in the authors' own count: ``The IOI circuit uses 26 attention heads grouped into 7 main classes to identify and output the indirect object in sentences of the form `When Mary and John went to the store, John gave a drink to \underline{\hspace{1cm}}'\,.'' The seven classes are Previous Token, Duplicate Token, Induction, S-Inhibition, Name Mover, Negative Name Mover, and Backup Name Mover. Description mode: implementational-functional. Every cell below was re-derived from the origin paper read in full, with each post-origin result checked against its own source.

{\footnotesize
\renewcommand{\arraystretch}{1.08}
\begin{longtable}{@{}lll>{\raggedright\arraybackslash}p{10.0cm}@{}}
\caption{Full 35-criteria audit of the IOI circuit. Status: \textbf{C} = Confirmed, \textbf{PC} = Partially confirmed, \textbf{U} = Untested, \textbf{I} = Inconclusive, \textbf{D} = Disconfirmed.}
\label{tab:ioi-audit}\\
\toprule
...
C1 & Falsifiability & C & Twenty-six heads are named with role assignments, each making a path-patching prediction that could have failed. The three validation criteria are stated as quantities with failure conditions before they are computed, and the circuit fails one of them (their \S4.1, greedy incompleteness up to 3.09) \\
C2 & Structural plausibility & PC & Weight-level analysis exists for one head class: the OV copy score reaches $>$95\% for all three Name Movers against $<$20\% for an average head, and 98\% negative copy score for the Negative Name Movers (their \S3.1). For four of the seven classes it is explicitly not done---\S3.3 lists QK collision detection, OV positional copying and parameter-level induction analysis as ``checks that we omitted due to time constraints,'' and \S5 states the S-Inhibition Heads' attention patterns are not understood \\
C3 & Convergent validity & PC & Five families at origin: path patching, attention-pattern inspection, weight-level copy scores, out-of-distribution validation on repeated random tokens (their Appendices H, I), and a behavioral adversarial test (\S4.4). Later agreement is broad---ACDC \citep{conmy2023towards}, attribution patching \citep{syed2023attribution}, EAP-IG \citep{hanna2024faith}, SAE circuits \citep{oneill2024sparse} and edge pruning \citep{bhaskar2024finding} recover overlapping head sets. The cap is that the agreement is method-conditional: varying the ablation or the metric changes the recovered set \\
C4 & Discriminant validity & I & Tested twice with opposite results in different models. \citet{merullo2024circuit} find 25/32 $=$ 78\% head overlap between the IOI and Colored Objects circuits in GPT-2 \emph{Medium} at a top-2\% threshold, against 3/18 for Greater-Than as a control; \citet{hanna2024faith} find cross-task faithfulness near zero for most task pairs in GPT-2 \emph{small}, with the smallest circuit containing $\geq$90\% of the manual IOI nodes reaching 0\% faithfulness. Different models, different quantities, unreconciled \\
C5 & Nomological validity & PC & Connected to induction-head theory through its operational definition rather than by analogy: their Appendix H applies the prefix-matching and copying scores of \citet{olsson2022context} on repeated random sequences, and heads 5.5 and 6.9 rank among the top five induction heads. The theory does not survive contact---``in the context we study, Induction Heads perform a different function compared to what was described in their original discovery,'' serving as positional signals (\S3.3), a mechanism the paper does not explain \\
C6 & Complementation validity & U & Unattempted. The circuit is carved into seven named classes, and no test establishes that the carving is functional. The class-level knockouts in \S3.4 remove one class at a time, which leaves a set of distinct complementation groups indistinguishable from a single group whose members share one role. The trans comparison needs two ablation runs and a composition rule, and the cis control is available in the same design \\
\midrule
\multicolumn{4}{@{}l}{\textit{Measurement Validity}} \\
M1 & Reliability & I & Partial at origin: standard deviations are plotted for Name Mover attention probabilities (their Figures 4c, 10) and $N$ is stated for every estimate ($N > 200$ path-patching pairs, $N = 500$ and $N = 1000$ for copy scores, 100{,}000 for the model baseline). No bootstrap or confidence interval on faithfulness. Measured later, with a middling answer: \citet{meloux2025variance} report mean pairwise Jaccard 0.67 under bootstrap resampling on GPT-2 small IOI, against the 0.8 bar they propose, rising to 0.88--0.89 under paraphrase resampling \\
M2 & Baseline separation & PC & Three baselines exist and separate. A na\"ive baseline circuit is built and scored (\S4.3, faithfulness 0.1 against the full circuit's 0.46). Ten uniformly-random subsets are sampled in the completeness test (their Figure 6). Copy scores carry an average-head baseline ($>$95\% vs $<$20\%; 98\% vs 12\%). Missing is a capacity-matched random \emph{circuit} null for the 87\% figure; \citet{shi2024hypothesis} supply it later, finding IOI significantly more faithful than random circuits up to 90\% of model size \\
M3 & Stability & D & Tested by independent groups and failed. \citet{miller2024transformer} vary six methodological choices and the same quantity ranges from below 0\% to well over 100\%, with the variant that ``best represents the hypothesis of Wang et al.'' scoring a median well over 100\%. \citet{zhang2023towards} report that symmetric token replacement and Gaussian noising ``detect different sets of heads as important'' for any fixed metric. \citet{meloux2025variance} find that across estimator and aggregation choices the circuit ranges from 5 to 21 edges while circuit error stays flat at 0.10--0.14 \\
M4 & Calibration & PC & Logit difference is well chosen---it has a sign, a null value, and a stated model baseline of 3.56. Faithfulness as $|F(M) - F(C)|$ is not calibrated: it has no upper bound, is reported as a single average, and \citet{miller2024transformer} show its inter-quartile range across examples reaches 50\% with outliers ``tens of thousands of percent.'' \citet{mueller2025mib} replace it with area-under-curve measures for this reason \\
M5 & Sensitivity & U & Unattempted. No known-positive control: no synthetic IOI circuit of known extent is planted in GPT-2 small and recovered. The nearest instruments arrived later and target the discovery \emph{method} rather than this circuit---\citet{conmy2023towards} introduce reset networks as a hallucination control, and \citet{mueller2025mib} substitutes a purpose-trained model whose IOI-like circuit is known by construction \\
M6 & Invariance & D & Tested and failed on the origin's own template set. \citet{miller2024transformer} report that faithfulness is ``systematically greater for the prompts of form BABA than prompts of form ABBA.'' \citet{franco2026finding} find the same split in the traced mechanism---per-prompt IOI circuits cluster into an ABBA cluster and a BABA cluster, with no substructure from surface wording. \citet{zhang2023towards} find that under one corruption scheme every metric misses at least two of the three Name Movers, while under another all three are found and no S-Inhibition Heads are \\
M7 & Selection correction & U & Unattempted. Eight Backup Name Movers are chosen from the post-knockout sweep because they ``were not part of any other groups, and affected the logit difference with an effect size above the threshold of 2\%''---the paper writes ``we arbitrarily chose.'' The candidate pool (144 heads) and $N$ are stated at every step; no multiplicity control is applied to any of it \\
\midrule
\multicolumn{4}{@{}l}{\textit{Internal Validity}} \\
I1 & Necessity & PC & Weak at the component level, and the paper says so. Knocking out all three Name Mover Heads costs ``only 5\% drop in logit difference'' (\S3.4). Minimality scores are conditional on a co-ablated set chosen to maximize them, and even so ``the individual contribution of some attention heads is small'' (\S4.2, Figure 7, all $\geq$1\%). \citet{gong2026conditional} quantify the reason: the name-mover module is 1.9$\times$ super-additive, so removing primaries costs 0.22 logit difference while removing primaries plus backups costs 1.15 \\
I2 & Sufficiency & PC & Mean-ablating everything outside the circuit leaves $|F(M)-F(C)| = 0.46$, ``or only 13\% of $F(M)=3.56$'' (\S4), which is the 87\% figure. It holds on average and under one protocol. Per example it does not: \citet{miller2024transformer} report an inter-quartile range up to 50\%, and \citet{uitdebos2024adversarial} find the model-circuit KL over $10^6$ input pairs has mean 5.15 and maximum 14.64. \citet{shi2024hypothesis} find IOI passes a relaxed sufficiency test up to 90\% of model size \\
I3 & Minimality & I & Tested at origin and passed on its own terms---every node has ``at least 1\% of the original logit difference'' impact (\S4.2, Figure 7), with the co-ablated sets printed in their Appendix K. Contested since, and the disagreement is traceable to the ablation method: \citet{shi2024hypothesis} remove about 20\% of IOI's edges with faithfulness non-monotonic in the removal, while \citet{lijanson2024optimal} find that under \emph{optimal} ablation the manual circuit is ``approximately optimal for its size'' and attribute the apparent slack under mean and resample ablation to spoofing artifacts \\
I4 & Specificity & I & Tested, with results that depend on the quantity measured. \citet{merullo2024circuit} find 78\% head overlap between the IOI and Colored Objects circuits in GPT-2 Medium and raise Colored Objects accuracy from 49.6\% to 93.7\% by forcing IOI-style inhibition; \citet{hanna2024faith} find cross-task faithfulness near zero in GPT-2 small and conclude ``overlap is not a good predictor of cross-task faithfulness.'' Shared components, unshared behavior \\
I5 & Rival mechanism exclusion & I & Rival sets exist and are not excluded. \citet{wang2023interpretability} report a na\"ive circuit reaching comparable faithfulness (\S4.3) and greedy search finding subsets with incompleteness score ``up to 3.09 (87\% of the original logit difference)'' that ``were usually not semantically interpretable'' (\S4.1). \citet{chen2026circuits} push it further: two IOI subgraphs at 100\% accuracy with edge overlap 4.1\%, and a shared core of 11 edges out of a 7{,}382-edge union across twenty of them---with the caveat that node-level overlap is 64.2\%, so the near-disjointness lies in the routing \\
I6 & Double dissociation & U & Unattempted, and searched for: no published double-dissociation test on IOI has been located. The nearest published design runs the full cross-task ablation matrix on other models and finds no dissociation at all---\citet{lisubramani2026howmuch} report that ``ablating one task's circuit damages another task's performance about as much as that task's own circuit does,'' with IOI the extreme case \\
I7 & Confound control & PC & Two confounds controlled, and one of them is a main-text section. Position versus token identity: \S3.2 and their Appendix A build six counterfactual datasets isolating the two signals and report the decomposition (their Figure 9), then \S3.3 locates the position signal upstream, with the converse control showing that changing the repeated-name tokens at fixed position moves logit difference by ``less than 8\%'' of the S-Inhibition effect. Syntax: 15 templates in both orders, with mean ablation computed per template ``to ensure that grammatical information is constant'' (\S2.1). Name frequency and sequence length are uncontrolled, at origin and since \\
I8 & Confounding sensitivity & U & Unattempted. No E-value or sensitivity analysis for an unmeasured confounder \\
I9 & Epistatic interaction & PC & Non-additivity is discovered, named, and made structural. \S3.4 re-runs the whole path-patching sweep after knocking out a head class and finds heads change role---Negative Name Mover 10.7 flips sign (their Figure 15)---which is what Backup Name Movers are. The minimality criterion is itself defined as a conditional effect $|F(C \setminus (K \cup \{v\})) - F(C \setminus K)|$, with the co-ablated set printed per head. No systematic pairwise grid, and the greedy completeness sets that mix classes are ``difficult to interpret'' (\S4.1) \\
I10 & Rescue reversibility & PC & Not run at origin---path patching writes corruption into a clean run rather than restoring a corrupted one, so the direction is reversed. Run later at the training level: \citet{chhabra2025neuroplasticity} corrupt GPT-2 small by fine-tuning on corrupted IOI and find that retraining recovers the original circuit at 95--96\% faithfulness, and that clean fine-tuning amplifies existing mechanisms without introducing new ones \\
I11 & Onset coupling & PC & Untestable in GPT-2 small, which ships no pretraining checkpoints, though Stanford CRFM offers reproduction GPT-2 checkpoints. In Pythia, \citet{tigges2024llm} track 154 checkpoints and find name-mover heads emerging at 2--8 $\times 10^9$ tokens ``during or just before IOI behavior appears'' \\
I12 & Offset coupling & PC & One Pythia-160m head \emph{loses} name-mover behavior at 3 $\times 10^{10}$ tokens while the algorithm persists in other heads \citep{tigges2024llm}, and \citet{lisubramani2026howmuch} find IOI circuit reuse in OLMo-2 peaking in the first $\sim$76B tokens and then declining. Observed rather than induced, and in other models \\
\midrule
\multicolumn{4}{@{}l}{\textit{External Validity}} \\
E1 & Intervention reach & I & One ablation \emph{value} at origin, applied at three granularities: path patching (edge-level), knockout (node-level, \S2.1), and activation patching of a whole head class with the forward pass recomputed. Zero ablation is tried and rejected on evidence---``0 is an arbitrary value \ldots\ we find zero ablation to lead to noisy results in practice'' (\S2.1). The number moves with the primitive, in both directions: \citet{miller2024transformer} find resample ablation systematically lower than mean at the node level ($p = 10^{-5}$), while the edge-level, specific-position variant that ``best represents the hypothesis of Wang et al.'' has ``a median score well over 100\%.'' No steering, no weight intervention, no dose-graded ablation at origin \\
E2 & Prompt generalization & D & Tested at scale on the origin task and failed. \citet{uitdebos2024adversarial} form $10^6$ clean/corrupted pairs on GPT-2 small and report model-vs-circuit KL with mean 5.15, median 5.12, maximum 14.64 and worst points ``more than 5 standard deviations away from the mean,'' concluding that the IOI circuit fails ``even on completely benign inputs from the original task.'' The failures have a content signature---the worst pairs ``often seem to involve romantic items'' \\
E3 & Cross-task generalization & PC & Present at origin in one direction: their Appendix H shows Name Mover, Backup and Negative heads writing in the next-token direction on repeated random sequences, ``suggest[ing] that these heads are involved beyond the IOI task.'' Later, \citet{merullo2024circuit} raise Colored Objects accuracy from 49.6\% to 93.7\% by forcing IOI-style inhibition, and \citet{mcdougall2023copy} show the negative name mover is a general copy-suppression mechanism (76.9\% of L10H7's effect on OpenWebText). Against this, \citet{hanna2024faith} report near-zero transfer on most task pairs \\
E4 & Cross-model generalization & PC & The origin's own preliminary GPT-2 medium analysis reports a sparse set of direct-effect heads but ``not all of these heads attend to IO and S'' (\S5). \citet{merullo2024circuit} reproduce the circuit in GPT-2 Medium with ``minor differences,'' the negative mover behaving differently. \citet{tigges2024llm} reproduce the three-step algorithm across Pythia 70m--2.8b with step metrics ``generally above 50\%,'' with copy-suppression heads flipping sign. \citet{mueller2025mib} localize IOI in four model families. Component overlap with other tasks falls to 5/10 heads in GPT-2 Large and 0/10 in XL \\
E5 & Graded response & PC & Graded in circuit \emph{extent} rather than intervention \emph{strength}, at origin and since. Their Figure 6 plots $F(C \setminus K)$ against $F(M \setminus K)$ over many subsets of different sizes, and the greedy procedure records the intermediate sets. \citet{mueller2025mib} integrate faithfulness over circuit size to define its headline measures, and \citet{shi2024hypothesis} sweep edge removal and find IOI's faithfulness ``does not vary monotonically as more edges are knocked out.'' No interpolation between clean and ablated activations \\
E6 & Novel prediction & C & \S4.4 is a novel prediction in the strict sense, with a matched control. From duplicate detection, the mechanism predicts that duplicating the \emph{indirect} object in a natural sentence should break the model while duplicating the \emph{subject} should not. Both halves hold: the rate at which the subject beats the indirect object rises from 0.7\% to 23.4\% with an extra indirect object and \emph{falls} to 0.4\% with an extra subject, with logit difference moving to 1.23 and 3.64 respectively from a baseline of 3.55. The authors hedge only whether the circuit was needed to generate the prediction \\
\midrule
\multicolumn{4}{@{}l}{\textit{Interpretive Validity}} \\
V1 & Level declaration & PC & Circuits are formally defined (\S2.1, an induced subgraph of the computational graph with knockout semantics) and the algorithm is stated as three steps mapped onto three head classes (\S3). The implementational--functional level is never named as such, and the paper moves between ``the circuit performs IOI'' and ``these heads implement this algorithm'' without marking the shift \\
V2 & Level-evidence match & PC & Evidence supports the topographic claim (which heads, at which positions, in which order) and the information-flow claims (what each reads from where). The algorithm-level claim is supported for the S-Inhibition step by the token/position decomposition and asserted for the Duplicate Token and Induction steps, whose parameter-level validation \S3.3 states was not done. \citet{palumbo2024validating} decline to evaluate the circuit against their axioms for exactly this reason: the paper ``leaves key components of the interpretation unspecified'' \\
V3 & Alternative level & I & A simpler description is tested at origin and is not clearly worse: the na\"ive circuit reaches faithfulness 0.1 against the full circuit's 0.46 (\S4.3), and is separated only by the by-class and random completeness tests, since ``when we applied the greedy heuristic \ldots\ both circuits have similarly large incompleteness scores.'' A more general description arrived later and is supported: \citet{merullo2024circuit} argue the mechanism ``boils down to a more generic algorithm for copying from a list of potential options, rather than strictly indirect object identification'' \\
V4 & Unlicensed labeling & PC & Most labels name measured operations---``Name Mover'' names a copy, ``Duplicate Token'' names a match, ``Previous Token'' names an attention pattern. Two do more work than the evidence: ``S-Inhibition'' imports a neuroscience mechanism term for what is measured as an attention-probability shift, and the Negative Name Movers are said to ``help the model \emph{hedge} so as to avoid high cross-entropy loss,'' which the paper marks as speculation and \citet{mcdougall2023copy} later replaces with copy suppression. No label is contrastively tested \\
V5 & Scope declaration & C & Declared in the abstract (``they also point to remaining gaps in our understanding''), in \S1 (``fails to pass the most challenging tests''), in \S3 (MLPs, layer norms and embeddings excluded by design), in \S3.3 (``Missing validations and caveats''), in \S4.4 (``Limitations of the attack''), and in \S5, which names the scale limit and frames the work as a model organism: ``these highly detailed (though narrow) examples'' \\
\midrule
\multicolumn{4}{@{}l}{\textbf{Total:} 3 Confirmed, 18 Partially confirmed, 7 Inconclusive, 5 Untested, 3 Disconfirmed} \\
\multicolumn{4}{@{}l}{\textbf{Verdict: Causally Suggestive}} \\
\end{longtable}
}

\paragraph{Summary.} Of 36 criteria: 3 Confirmed, 18 Partially confirmed, 7 Inconclusive, 5 Untested, 3 Disconfirmed. The five Untested criteria (C6, M5, M7, I6, I8) are all \emph{unattempted} rather than untestable---the hybrid-pass complementation test (C6) requires only two ablation runs and a composition rule, planting a known circuit in GPT-2 small is possible, M7 requires only recording comparisons the paper already makes, and neither I6 nor I8 needs an instrument the field lacks.

The profile explains the verdict. Seven Inconclusive rows are not seven gaps; they are seven criteria on which the published evidence answers in both directions depending on the model, the ablation value, or the quantity being scored. The three Disconfirmed rows sit in measurement (M3 stability, M6 invariance) and external validity (E2 prompt generalization), and the dependency chain makes measurement a ceiling on the internal-validity claims above it. Rival head sets reaching comparable faithfulness (I5) bear on uniqueness, which the lenient reading leaves unasserted, so they do not move the tier. Necessity via intervention (I1) and baseline separation (M2) place the claim at \textbf{Causally Suggestive}, and E1 holds it there: promotion requires agreement across ablation methods, and \citet{miller2024transformer} move the headline quantity from below 0\% to well over 100\% across six methodological choices.

Concrete next steps follow from the untested criteria. Discriminating among rival circuits (I5) is now the binding constraint, and it needs a test that scores subgraphs against each other rather than each against the full model. Behind it: (1) \textbf{I6 double dissociation}---no published test exists, and the nearest design finds cross-task ablation damaging other tasks about as much as their own circuits do \citep{lisubramani2026howmuch}; (2) \textbf{M7 selection correction}---the candidate pool and $N$ are stated at every stage, so a multiplicity correction requires no new experiment; (3) \textbf{M5 sensitivity}---plant an IOI-like circuit of known extent and check that the pipeline recovers it; (4) \textbf{I7 confound control}---name frequency and sequence length remain untested at origin and since. Cross-model replication, listed as an open item in earlier versions of this audit, has since been run in GPT-2 Medium, Pythia and four further model families.

% ============================================================
\subsection{Circuit specification: IOI}
...
The specification below covers six of the seven head classes \citet{wang2023interpretability} identify; Backup Name Movers are omitted because their role is defined by response to ablation rather than by a forward-pass edge, so they carry no prediction of the form the pipeline tests. It defines a 6-step computational DAG:
...
\citep{miller2024transformer} find the faithfulness numbers to be method-conditional, and \citep{chen2026circuits} find alternative circuits at equal accuracy sharing almost none of their edges.
...
\midrule
Calibration Checks & Intervention robustness: the headline faithfulness value moves with the ablation protocol in both directions \\
\midrule
Criterion Tested & I1 necessity, I2 sufficiency, and E1 intervention reach \\
\midrule
Result & The circuit recovers 87\% of the full-model logit difference under mean ablation on average; across six methodological choices the same quantity ranges from below 0\% to well over 100\% \citep{miller2024transformer} \\
\midrule
Status & I1 partial---knocking out all three name movers costs a 5\% drop, because backup heads take over; I2 partial, holding on average and not per example; E1 inconclusive, because intervention variants disagree in sign \\
\midrule
Failure Mode Addressed & Cherry-picking metrics and method-conditional evidence \\
\midrule
Verdict Implication & Does not support Mechanistically Supported, since that tier requires agreement across ablation methods. This is the criterion that caps the claim at Causally Suggestive \\
\bottomrule
\end{tabular}
