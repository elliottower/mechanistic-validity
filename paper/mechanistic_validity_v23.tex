% Mechanistic Validity: A Validity Theory and Evidence Standard for Mechanistic Claims
\pdfoutput=1
...
% claims are numbered subsubsections here, so each one appears in the table of
% contents and is reachable by hyperlink
\newcommand{\claimheading}[1]{\subsubsection{#1}}
...
to human-interpretable concepts \citep{cunningham2024sparse, bricken2023monosemanticity}. The field has also begun to
...
Mechanistic interpretability screens large candidate pools and reports the
winners.  A circuit paper ranks 144 attention heads and names the few that clear a
threshold; a feature paper scores thousands of latents and interprets the top ones;
a direction paper searches every layer crossed with every position.  The pool is
usually disclosed, and the selected item's score is then reported as a property of
the model rather than of the search.  Across the sixteen claims audited here, not
one recomputes a headline number on an unselected slice, so nothing separates the
effect from the maximum of the pool it was drawn from.
...
Recent work applies construct validity theory to ML evaluation more broadly. \citet{bean2025measuring} find that only 16\% of 445 reviewed LLM benchmarks conduct any statistical testing---a striking parallel to our finding that most MI claims lack measurement validity (M1--M7). Their own diagnosis is that ``key concepts are often poorly defined or operationalized, limiting the reliability of the conclusions they draw.'' We would add a second dynamic, on our own account rather than theirs: once a metric becomes conventional, continued use can substitute for evidence that it is calibrated, which is how faithfulness and IIA come to serve as primary evidence in mechanistic interpretability without calibration checks. \citet{freiesleben2025benchmarking} develop an argument-based construct-validity framework for benchmark evaluation, following Messick and Kane. Their fourth step is close to ours: in light of the evidence, scientists ``must then constrain the scope and content of the intended inference accordingly,'' and validity on their account is gradual, with ``more substantial or wide-ranging inferences from benchmark scores'' requiring ``stronger validity conditions.'' Two differences remain, and the second is a disagreement rather than a gap. The unit of analysis here is a mechanistic claim, a causal assertion about how a model works rather than a task-performance score. And they hold that their scheme ``does not imply a strict hierarchy between types of validity and validity conditions,'' where this framework orders the five types by dependency, on the ground that a failure early in the chain limits what later evidence can establish. Where Freiesleben et al.\ ask ``does this benchmark measure the right thing?'' and Bean et al.\ ask ``is this benchmark statistically sound?,'' Mechanistic Validity asks ``does the evidence pattern support the causal claim?''

Assurance cases are the nearest infrastructure in another direction. Claims-Arguments-Evidence \citep{bloomfield2014cae} and the Goal Structuring Notation \citep{kelly2004gsn} decompose a top-level claim into subclaims until, in the latter's terms, ``a point is reached where claims can be supported by evidence,'' which is structurally similar to what an audit produces. They differ in purpose: an assurance case is built by a system's proponent to argue that a specific claim holds, and its confidence measures ask how much the argument's structure justifies belief. An audit is built by a third party against a claim already published, and must handle the case where the proponent's own statement of the claim is ambiguous.

% ------------------------------------------------------------------
\subsection{Graded claims}
\label{sec:related-readings}

The observation that claims carry variable strength is old, and the vocabulary for it is well developed. \citet{toulmin1958uses} makes the \emph{qualifier} a component of argument structure precisely so that a claim's force is stated rather than assumed. \citet{niiniluoto1987truthlikeness}, developing Popper, replaces a binary confirmed/disconfirmed verdict with graded truthlikeness across theories. Each of these treats strength as a scalar attached to a proposition, or as a comparison between distinct theories. Four recent proposals do arrange one claim by inferential strength, and the difference from each is narrower than it first appears. \citet{leavitt2020falsifiable} walk a single hypothesis from weak to strong, each step adding a dissociation, a stated baseline or a competing hypothesis. \citet{gupta2026anthropomorphic} define three evidence levels that are ``not levels of experimental sophistication, but levels of what a result allows researchers to claim.'' \citet{salaudeen2025measurement} open with one measurement supporting two claims of different reach, holding that the standard of evidence ``depends on the conceptual gap between the measurement and the object of the claim, with broader gaps demanding stronger justification.'' And \citet{jacovi2020faithfully} call for ``discarding the binary notion of faithfulness in favor of a more graded one,'' grading it across the input space. What is distinctive here is that readings are derived from what a given paper asserted rather than fixed by domain, that they vary along two stated axes, and that every reading is scored against one evidence record, so the strongest supported reading can be reported without erasing the stronger formulations the paper or its citers use elsewhere.

Two nearer neighbors come from evidence-based medicine, and one of them is close enough that we adopt it as precedent rather than distinguish it. The surrogate-outcome literature scores the relationship between a surrogate and a patient-centered outcome against three explicitly separate levels of evidence: Level~3, biological plausibility from pathophysiological reasoning; Level~2, a consistent association from observational or interventional studies; Level~1, evidence that treatment-induced changes in the surrogate correspond to treatment-induced changes in the final outcome \citep{euhta2024outcomes}. These are three different propositions about one relationship rather than three quality grades of one proposition, satisfying a weaker level establishes nothing about a stronger one, and papers are routinely criticized for conflating them. That is the structure this paper generalizes. It differs from ours in that its levels are fixed by domain---every surrogate claim is scored against the same ladder---whereas readings are derived per claim from what the paper asserted, and vary along two axes: mechanistic specificity and scope.
...
Four things are new, and each is narrower than the section that introduced it
might suggest.  The first is the object.  A test score, a study design, a
treatment effect and a gene--disease assertion each have a settled propositional
content an assessor can take as given, where a mechanistic claim has parts, an
organization, a level of description and a scope that can each be asserted at
more or less strength independently of the others.

The second is the reading ladder, and it is the most heavily populated
neighborhood: four recent proposals arrange one claim by inferential strength,
and \Cref{sec:related-readings} states what survives contact with them---readings
derived from what a given paper asserted rather than fixed by domain, varying
along two stated axes, and scored against one evidence record.

Three of the interpretive criteria have no counterpart we have located in the validity frameworks surveyed above. V1 asks whether a claim declares the description mode at which it is pitched, V2 whether its evidence supports a claim at that level, and V3 whether a simpler description would account for the same evidence. The remaining two are less novel and we do not claim them: scope declaration (V5) is a standard requirement of medical reporting guidelines, and unlicensed labeling (V4) is arguably a special case of construct validity.
...
Existing infrastructure evaluates complementary parts of the interpretability pipeline. MIB \citep{mueller2025mib} runs two tracks: one scores circuit-discovery methods against each other, and against a known edge-level circuit in a compiled setting; the other scores featurization methods---sparse autoencoders, distributed alignment search, principal components---by interchange intervention accuracy against a high-level causal model, and reports that SAE features do not beat raw neurons. Both tracks fix the claim in advance, since the task defines what is to be localized and the method that localizes it best wins. Tracr \citep{lindner2023interp} compiles programs into transformer weights, supplying the ground truth such comparisons consume rather than performing one; its authors ask that evaluations built on compiled models be treated ``as a minimum bar rather than a full validation of a technique.'' Causal scrubbing \citep{chan2022causal} and the hypothesis tests of \citet{shi2024hypothesis} come closer, taking a supplied hypothesis and asking whether it survives: the first replaces activations according to a stated correspondence, the second applies formal tests for mechanism preservation, localization and minimality to six published circuits. Mechanistic Validity takes the output of these pipelines as its input. Given a claim already published, it asks which readings of that claim the whole evidence record supports, and audits how the hypothesis was formulated, what alternatives were excluded, and how far it was said to reach.

SAEBench \citep{karvonen2025saebench} audits SAE evaluation metrics; Mechanistic Validity treats that audit as evidence about its own measurement-validity criteria (M1--M7). MechEvalAgent \citep{bai2026mecheval} scores mechanistic interpretability studies on coherence, reproducibility and generalizability through a twenty-three item checklist, several of whose items are validity criteria under other names. Mechanistic Validity centers the readings a published claim admits and the evidence supporting each. \citet{mahale2026causally} likewise grounds interpretability in causal analysis, but its target is the \emph{faithfulness of a natural-language explanation} of an already-discovered circuit, scored with ERASER-style sufficiency and comprehensiveness; Mechanistic Validity instead asks whether the evidence pattern supports the causal \emph{claim} itself, decomposed across all five validity types rather than the single axis of explanation faithfulness.
...
The demand for validity evidence is now being made by funders, without an instrument to meet it. The Schmidt Sciences Trustworthy AI research agenda asks for evaluations that ``measure meaningful latent safety properties (construct validity)'' and ``predict deployment behavior (predictive validity),'' and poses as an open problem what it calls evidence standards for decision-relevant evaluations: ``what structured evidence is sufficient to justify relying on it in a safety case? What kinds of validity evidence are needed, how should such arguments be stress-tested, and what would falsify the claim that this is sufficiently reliable for deployment decisions?'' \citep{schmidt2026trustworthy}. That agenda is about safety evaluations and scopes interpretability to a separate program, so it is not a standard this paper competes with. The question transfers: for a mechanistic claim, the criteria are the structured evidence, the tier is what that evidence licenses, and an unmet criterion names what would falsify a stronger reading.
...
\item \textbf{Synthesis.} How is evidence aggregated across methods? Currently it is not: separate discovery methods return overlapping but non-identical head sets, and no procedure adjudicates between them. A synthesis model would first have to establish whether those methods are noisy measurements of one latent target or sound measurements of different targets; consensus voting assumes the former and would erase the disagreement in the latter case. The audit records the disagreement rather than collapsing it.
...
\midrule
No baseline control & Report high score without testing a random or untrained model & M2 Baseline\newline separation & SAEBench: Targeted Probe Perturbation scores \emph{higher} on a randomly initialized model than on the trained one \citep{karvonen2025saebench} \\
\midrule
...
The Mechanistic Validity framework evaluates a mechanistic claim through seven layers, in pipeline order. Each layer answers one question, and each takes the previous layer's answer as its input:
...
\centering
\small
\caption{Seven description modes, ordered from most committal (computational) to least committal (topographic). Along that sequence a higher mode requires the evidence of the modes below it plus bridging evidence; implementational-statistical claims sit partly outside the sequence, since they characterize activation distributions rather than asserting more about a component. Implementational-statistical is partially orthogonal: it characterizes activation distributions rather than making structural commitments.}
\label{tab:modes}
...
The top tiers require more than accumulated confirmations. A claim tested only on
the evidence it was constructed to fit has been confirmed where it needed to be
corroborated: it passed a test it could not have failed. Following the
error-statistical tradition \citep{mayo2018statistical}, the criteria that lift a
claim to Triangulated or Validated must be established by \emph{severe} tests,
meaning tests the claim could have failed and did not.

Severity enters this framework through the tier requirements themselves.
Convergent evidence across families with different failure modes (C3) is, by
construction, evidence a claim was not optimized to produce, and Triangulated
already demands it. A claim resting on one family therefore stops at
Mechanistically Supported however often it is re-tested, and that ceiling follows
from what the tier asks for. No separate severity gate is added on top.

Preregistration is recorded and never scored. A prediction fixed before the
result is more severe than one selected after it, and the audit records which a
test was. The status the criterion receives is the same either way. Scoring it
would make identical evidence count differently according to whether a disclosure
was made, and this framework grades evidence. Selection correction (M7) is
recorded on the same footing and for the same reason: both describe how a result
was obtained, and both belong in the record rather than in the tier.
...
The verdict assignments for all sixteen claims appear in the audit supplement, where each claim receives a full 36-criterion table, a readings table and a tier ladder. The tier system discriminates: the sixteen claims spread across five labels, and each difference is traceable to a named criterion rather than to an overall impression. Two claims from the same paper land at opposite ends---induction heads reach Triangulated for token copying and Disconfirmed for general in-context learning---which is the clearest demonstration that the unit of assessment is the claim and not the paper.
...
This is a scope refutation the authors anticipated. Their Argument 6 states that ``above some size, non-induction composition heads could together account for more of the phase change and in-context learning improvement than induction heads do.'' No attempted replication failed: the narrow mechanism replicates, and the reach asserted for it does not. One paper, two claims, two tiers---the same structure as the workspace case, and the reason the audit scores claims rather than papers.

\paragraph{IOI Circuit (Causally Suggestive).} \citet{wang2023interpretability} identify 26 attention heads grouped into seven classes for indirect object identification. Mean-ablating everything outside the circuit leaves a logit-difference gap of 0.46, ``or only 13\% of $F(M) = 3.56$,'' which is the 87\% figure. Necessity at the component level is weaker than the number suggests, and the paper says so---knocking out all three Name Mover Heads costs ``only 5\% drop in logit difference'' (\S3.4), and the minimality scores are conditional on a co-ablated set chosen to maximize them.
...
Each claim is scored at its most lenient reading, and the lenient reading here is that these 26 heads carry the indirect-object computation. Rival subgraphs are compatible with it, since a second sufficient set establishes that the mechanism is not unique and uniqueness belongs to the stronger reading. Method disagreement (E1) caps the claim instead, and it caps every reading. The localization is real and reproducible, and which subgraph constitutes \emph{the} mechanism is open.
...
The claim ``chain-of-thought reasoning is faithful'' is at least four claims, each requiring different evidence.  Does CoT predict the model's output (\emph{behavioral faithfulness}, testable with perturbation studies)?  Does it converge with other interpretability methods on the same internal structure (\emph{convergent faithfulness}, requiring multi-method evidence)?  Does it describe the functional roles actually implemented by internal components (\emph{role faithfulness}, requiring independent functional tests)?  Or does it map onto the model's computational graph (\emph{structural faithfulness}, requiring causal and structural evidence)?  \citet{lanham2023measuring} and \citet{turpin2023unfaithful} demonstrate that behavioral faithfulness often fails: models produce plausible reasoning chains that do not reflect their actual decision process.  Under Mechanistic Validity, ``CoT is faithful'' at the behavioral level is a Proposed claim (behavioral evidence only, no causal intervention); at the structural level, it would require at minimum Mechanistically Supported status across internal and construct validity criteria.  The framework makes the gap between these levels explicit rather than leaving ``faithful'' as an ambiguous umbrella term.
...
\nocite{campbell1963experimental}
...
\clearpage
\subsection{Criterion audits}
\label{appendix:audits}
Each claim below carries its own scorecard against the 36 criteria, the readings the
claim admits, and the tier ladder showing what blocks promotion.  The claims appear in
the order they are introduced in \Cref{sec:case-studies}.
